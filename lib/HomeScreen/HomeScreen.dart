        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(onPressed: (){
              ...
              icon: Lottie.asset("assets/search2.json",) ,
...
class NewsSearch extends SearchDelegate{// extend to let me search
   /// List<String> _oldFilters =  [];
  @override  // when i click on icon what will be happen
  List<Widget>? buildActions(BuildContext context) {    // (1)
    return [
  ...
  Widget? buildLeading(BuildContext context) { //(2)
    return
      IconButton(onPressed: (){
        hideLoading(context) ;
      },
  ...
  @override
  Widget buildResults(BuildContext context) { //(3)
      ...
      builder: (_, snapshot) {
        ...
        return Expanded(
          child: ListView.builder(
            itemBuilder: (_, index) {
              return Container(
                  decoration: BoxDecoration(border: Border.symmetric(horizontal: BorderSide(color: Colors.brown))),
  ...
  //-----------------------------------Suggestions------------------------------------------------------------
  @override
  Widget buildSuggestions(BuildContext context) {//(4)
  return Center(child: Image.asset('assets/search.gif',fit: BoxFit.fill,));
  }


}
//const Center(child: Text('Type what you need to search ',style: TextStyle( fontSize: 30,color: Colors.red),));